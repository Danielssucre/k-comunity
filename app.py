...
def setup_database():
    ...
    CREATE TABLE IF NOT EXISTS questions (
        ...
        opciones TEXT NOT NULL, -- Almacenado como JSON/string delimitado
    ...
    cursor.execute("SELECT * FROM users WHERE username = 'daniel.suarez'")
    admin = cursor.fetchone()
    if not admin:
        admin_pass_hash = pwd_context.hash("Sentria2030*")
        cursor.execute("INSERT INTO users (username, password_hash, role) VALUES (?, ?, ?)",
                       ("daniel.suarez", admin_pass_hash, "admin"))
...
def show_login_page():
    ...
    with st.form("register_form", clear_on_submit=True):
        ...
        if reg_submitted:
            ...
            elif new_username == "daniel.suarez":
...
def show_admin_panel():
    ...
    for user in cursor.fetchall():
        ...
        if user['username'] != 'daniel.suarez':
